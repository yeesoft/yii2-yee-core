...
class DbMessageSource extends \yii\i18n\MessageSource
{
    ...
    protected function loadMessagesFromDb($category, $language)
    {
        ...
        $mainQuery->select(['t1.message message', 't2.translation translation'])
            ...
            ->where('t1.id = t2.source_id AND t1.category = :category AND t2.language = :language')
        ...
        if ($fallbackLanguage != $language) {
            ...
            $fallbackQuery->select(['t1.message message', 't2.translation translation'])
                ->from(["$this->sourceMessageTable t1", "$this->messageTable t2"])
                ->where('t1.id = t2.source_id AND t1.category = :category AND t2.language = :fallbackLanguage')
